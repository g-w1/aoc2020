...
pub fn main() !void {
    ...
    while (it.next()) |passport| {
        var p: Passport = .{};
        var itt = std.mem.tokenize(passport, "\n ");
        while (itt.next()) |field| {
            var ittt = mem.split(field, ":");
            const fnn = ittt.next().?;
            const payload = ittt.next().?;
            if (mem.eql(u8, "byr", fnn)) {
                const num1 = try std.fmt.parseUnsigned(u32, payload, 10);
                if (num1 >= 1920 and num1 <= 2002)
                    p.byr = true;
            }
            if (mem.eql(u8, "iyr", fnn)) {
                const num1 = try std.fmt.parseUnsigned(u32, payload, 10);
                if (num1 >= 2010 and num1 <= 2020)
                    p.iyr = true;
            }
            if (mem.eql(u8, "eyr", fnn)) {
                const num1 = try std.fmt.parseUnsigned(u32, payload, 10);
                if (num1 >= 2020 and num1 <= 2030)
                    p.eyr = true;
            }
            if (mem.eql(u8, "hgt", fnn)) {
                if (mem.endsWith(u8, payload, "cm")) {
                    const num1 = try std.fmt.parseUnsigned(u32, payload[0 .. payload.len - 2], 10);
                    if (num1 >= 150 and num1 <= 193)
                        p.hgt = true;
                }
                if (mem.endsWith(u8, payload, "in")) {
                    const num1 = try std.fmt.parseUnsigned(u32, payload[0 .. payload.len - 2], 10);
                    if (num1 >= 59 and num1 <= 76)
                        p.hgt = true;
                }
            }
            if (mem.eql(u8, "hcl", fnn)) {
                if (payload[0] == '#' and payload.len == 7) {
                    for (payload) |char| c: {
                        switch (char) {
                            '0'...'9', 'a'...'f' => p.hcl = true,
                            else => {
            ...
            if (mem.eql(u8, "ecl", fnn)) {
                if (mem.eql(u8, payload, "amb") or mem.eql(u8, payload, "blu") or mem.eql(u8, payload, "brn") or mem.eql(u8, payload, "gry") or mem.eql(u8, payload, "grn") or mem.eql(u8, payload, "hzl") or mem.eql(u8, payload, "oth")) {
                    p.ecl = true;
                }
            }
            if (mem.eql(u8, "pid", fnn)) {
                if (payload.len == 9) {
                    for (payload) |char| c: {
                        switch (char) {
                            '0'...'9' => p.pid = true,
            ...
            if (mem.eql(u8, "cid", fnn)) p.cid = true;
            std.debug.print("fnn: {}\n", .{fnn});
        }
        if (p.isValid()) {
            num_valid += 1;
    ...
    std.debug.print("num_valid: {}", .{num_valid});
}

const m: std.ComptimeStringMap = std.ComptimeStringMap();
